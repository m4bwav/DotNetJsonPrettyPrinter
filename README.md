...
CI (`.github/workflows/ci.yml`) builds, tests and packs on every push. To publish: bump `<Version>` in `JsonPrettyPrinterPlus.csproj`, tag the commit `v<version>` and push the tag; the `publish` job pushes the package to NuGet with the `NUGET_API_KEY` secret from the `nuget` environment.
